...
function NetRxDevice(app, opts) {
  ...
  process.nextTick(function() {
    ...
    {
    case "wlan0":
      var execcmdrx = 'rx1=`cat /sys/class/net/'+ opts.nic +'/statistics/rx_bytes`; sleep '+ opts.poll_interval +'; rx2=`cat /sys/class/net/'+ opts.nic +'/statistics/rx_bytes`; rx_bps=$((($rx2*8)-($rx1*8))); rx_kbps=$(($rx_bps / 1024)); echo $rx_kbps;'
      break;
    case "eth0":
      var execcmdrx = 'rx1=`cat /sys/class/net/'+ opts.nic +'/statistics/rx_bytes`; sleep '+ opts.poll_interval +'; rx2=`cat /sys/class/net/'+ opts.nic +'/statistics/rx_bytes`; rx_bps=$((($rx2*8)-($rx1*8))); rx_kbps=$(($rx_bps / 1024)); echo $rx_kbps;'
      break;
    ...
    case "l0":
      ...
      break;
    default:
      var execcmdrx = 'rx1=`cat /sys/class/net/'+ opts.nics +'/statistics/rx_bytes`; sleep '+ opts.poll_interval +'; rx2=`cat /sys/class/net/wlan0/statistics/rx_bytes`; rx_bps=$((($rx2*8)-($rx1*8))); rx_kbps=$(($rx_bps / 1024)); echo $rx_kbps;'
    }
