...
function NetTxDevice(app, opts) {
  ...
  this.G = "4"; // G is a string a represents the channel
  this.V = 0; // 0 is Ninja Blocks' device list
  this.D = 2000; // 2000 is a generic sandbox device
  this.name = 'Ninja NetMon ('+opts.nic+' | Tx | Kbps)';
...
//      });
    ...
    {
    case "wlan0":
      var execcmdtx = 'tx1=`cat /sys/class/net/'+ opts.nic +'/statistics/tx_bytes`; sleep '+ opts.poll_interval +'; tx2=`cat /sys/class/net/'+ opts.nic +'/statistics/tx_bytes`; tx_bps=$((($tx2*8)-($tx1*8))); tx_kbps=$(($tx_bps / 1024)); echo $tx_kbps;'
